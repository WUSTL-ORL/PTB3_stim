...
%
% startup.m file for user "scan" on Culver lab's SuperLogics stimulus
% computer.  Should be placed so that the file's full path is
% /home/scan/Documents/MATLAB/startup.m .
%
...
% Culver lab's stimulus code files.  Do not include trailing directory
% separator characters (e.g., use '/usr/share/stim', not
% /usr/share/stim/').
...
%% Change the current folder to the location where stimulus script outputs should be saved during scans.

if displayStatusMessages
   display('Changing current directory to the location where stimulus script outputs should be saved during scans.')
end

cd /home/scan/Scan_Stim_Output;

% Check whether there is a subfolder corresponding to the current date.  If
% not, then create that subfolder. Note that 'MM' corresponds to the
% 2-digit month for the datetime command but that 'mm' corresponds to the
% 2-digit month for the datestr command that converts datetime objects into
% strings.
currentDateString = datestr(datetime('now','Format','yyMMdd'),'yymmdd');
currentDateSubfolderPath = ['.' filesep currentDateString];  % Prepend './' (or '.\' on Windows) to check for current date subfolder only in current folder, not whole MATLAB search path.
if ~exist(currentDateSubfolderPath,'dir')
    mkdir(currentDateSubfolderPath);
end

% Change to the subfolder corresponding to today's date.
cd(currentDateString);

clear currentDateString currentDateSubfolderPath;
